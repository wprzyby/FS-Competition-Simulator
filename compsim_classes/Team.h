...
class Team
{
    ...
    double result_of_category(EventCategory category) const {return m_results.at(category);}

    // setters:
    void set_name(std::string name);
    void set_university(std::string university);
    void set_number(unsigned number) {m_number = number;}
    void set_category_result(EventCategory category, double result_to_set) {m_results[category] = result_to_set;}
