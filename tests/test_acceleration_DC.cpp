...
TEST_CASE("AccelerationEvent DC tests.", "[Driverless Cup]")
{
    ...
    EventResults results = event_simulator->simulate_event(acceleration_DC, teams);
    //

    // Creating map of correct results
    std::map<std::string, double> correct_results;
    correct_results.insert({"A", 75.0});
    correct_results.insert({"B", 63.12});
    correct_results.insert({"C", 3.75});
